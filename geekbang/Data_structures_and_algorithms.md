...
"""30| 图的表示：如何存储微博、微信等社交网络中的好友关系"""        
    ...
    1、如何理解“图”：
        ...
        图中的一个顶点可以与任意其他顶点建立连接关系。我们把这种
        连接关系叫做边(edge)。
        跟顶点相连接的边的条数，叫作顶点的度(degree)。
        
        我们把边有方向的图叫“有向图”，把边没有方向的图叫作“无向图”。
        在有向图中，我们把度分为入度(In-degree) 和出度(Out-degree)。
        顶点的入度，表示有多少条边指向这个顶点；
        顶点的出度，表示有多少条边是以这个顶点为起点指向其他顶点。
        
        还有一种，带权图(weighted graph)，每条边都有一个权重。
        
    2、邻接矩阵存储方法：
    
        图最直观的一种存储方法就是，邻接矩阵(Adjacency Matrix)。
        
        邻接矩阵的底层依赖一个二维数组，对于无向图来说，如果顶点 i 与顶点 j 之间
        有边，我们就把 A[i][j] 和 A[j][i] 标记为 1;
        对于有向图来说，如果顶点 i 到顶点 j 之间，有一条箭头从顶点 i 指向顶点 j 的边，
        那我们就将 A[i][j] 标记为 1。同理，如果有一条箭头从顶点 j 指向顶点 i 的边，
        我们将讲 A[j][i] 标记为 1。对于带权图，数组中就存储相应的权重。
        
        用邻接矩阵存储图，可以将很多图的运算转换成矩阵之间的运算。
        
    3、邻接表存储方法：
    
        另一种图的存储方法，邻接表(Adjacency List)。
        每一个顶点对于一条链表，链表中存储的是与这个顶点相连接的其他顶点。
