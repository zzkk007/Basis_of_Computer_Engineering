...
"""07| 复杂度来源：底成本、安全、规模"""  
    ...
    复杂度来源除了 高性能、高可用、高扩展性
    还有低成本、安全和规模。
    
    1、低成本：
    
        当我们设计“高性能”、“高可用”的架构时，同用的手段都是增加更多服务器
        来满足要求，而低成本正好相反，我们需要减少服务器数量才能达到低成本
        的目的。低成本本质上是与高性能、高可用冲突的，所以低成本很多时候不是
        架构设计的首要目标，而是架构设计的附加约束。
    ...
    2、安全：
        ...
        a. 功能安全：
            ...
            常见的 XSS 攻击、CSRF攻击、SQL注入、Windows漏洞、密码破解等。
            本质上因为系统实现有漏洞。功能安全其实就是“防小偷”。
            
            功能安全与具体的编码相关，与架构关系不大。
            
        b. 架构安全：
        
            架构安全就是“防强盗”
            传统架构主要依靠防火墙，防火墙最基本的功能就是隔离网络，
            通过将网络划分不同的区域，制定出不同区域之间的访问控制策略
            来控制不同信任区域传输的数据流。
            
        c. 规模：
        
            规模的复杂度主要原因就是“量变引起的质变”。
            

"""08| 架构设计三原则"""            

    架构设计主要目的是为了解决软件系统复杂度带来的问题，
    并分析了复杂度的来源。
    
    优秀程序员和架构师之间有一个明显的鸿沟需要跨越，
    这个鸿沟就是“不确定性”。
    ...
    1、合适原则：
        ...
        没有那么卓越的业务场景，却幻想灵光一闪为天才，是失败的第三个主要原因。
        
        真正优秀的架构都是在企业当前人力、条件、业务等各种约束下设计出现的
        能够合理的将资源整合在一起发挥出最大功效。
...
"""10| 架构设计流程：识别复杂度"""
    ...
    1、架构设计第一步：识别复杂度
        ...
        架构设计的本质目的是为了解决软件系统的复杂性，所以在设计
        架构时，首先要分析系统的复杂性。只有正确的分析了系统的复杂性
        后续的架构设计方案才不会偏离方向。
        
        将主要的复杂度问题列出来，然后根据业务，技术，团队等综合情况
        进行排序，优先解决当前面临的最主要的复杂度问题。
        
        使用排查法，对系统进行“高并发”、“高可用”、“高扩展性”排查。
        
        高性能主要从软件系统的未来的TPS和QPS:
            ...
            QPS：是Queries Per Second的缩写，意思是每秒查询率，
            是一台服务器每秒能够相应的查询次数，
            是对一个特定的查询服务器在规定时间内所处理流量多少的衡量标准。
            
            TPS即每秒处理事务数，包括：”用户请求服务器”、”服务器自己的内部处理”、
            ”服务器返回给用户”，这三个过程，每秒能够完成N个这三个过程，TPS也就是3；
            
            QPS基本类似于TPS，但是不同的是，对于一个页面的一次访问，形成一个TPS；
            但一次页面请求，可能产生多次对服务器的请求，服务器对这些请求，就可计入QPS之中。
...
"""11| 架构设计流程：设计备选方案""
    ...
    成熟的架构师需要对已经存在的技术非常熟悉，对已经验证的架构烂熟于心，
    然后，根据自己对业务的理解，挑选合适的架构模式进行组合，再对组合后
    的方案进行修改和调整。
    
    NoSQL: Key-Value 的存储和数据库的索引其实是类似的，Memcache 只是
    把数据库的索引独立出来做成了一个缓存系统。
    ...
    在《技术的本质》一书中，对技术的组合有清晰的阐述：
        新技术都是在现有技术的基础上发展起来的，现有技术又来源先前的技术。
        将技术进行功能性分组，可以大大简化设计过程，这是技术“模块化”
        的首要原因，技术的“组合”和“递归”特征，。
        
    虽说基于已有技术或者架构进行组合，然后调整，大部分情况能够得到我们需要的方案
    但并不意味着架构设计时一件简单的事情。因为可选方案很多，组合的方案很多。
    
    第一种常见的错误：设计最优秀的方案。
    
    第二种常见错误：只做一个方案。
    
        备选方案的数量以 3 ~ 5 个为最佳。少于 3 个方案可能是因为思维狭隘，考虑不全。
        多余 5 个则需要耗费大量的精力和时间，并且方案之间的差异不明显。
