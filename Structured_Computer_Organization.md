...
"-----------------------------------------------------------"
    ...
    3.1 门和布尔代数：
        ...
        数字电路只能表示两个逻辑值。一般用电压为 0 ~ 0.5v 的信号表示一个值(如二进制0)
        用电压为 1 ~ 1.5 的信号表示另一个值（如二进制 1）。这两个范围之外的电压值为
        非法值。
        
        一种我们称为“门”的微小电子设备，可以用来实现这两个逻辑值的多项运算，可以说“门”
        是制造所有数字计算机的硬件基础。
        
        生产“门”的主要技术有“双极晶体管” 和 “MOS”(金属氧化物半导体)
        双极晶体管主要有 TTL(晶体管-晶体管逻辑) 和 ECL(射极耦合逻辑)
        MOS 门电路比TTL 和 ECL 要慢一些，但要求能量要低一些，可以较大数量的封装在一起。
        MOS 有很大种类，主要有 PMOS、NMOS、CMOS
        绝大多数现代 CPU 和存储器使用的是 CMOS 技术。电源电压为 +1.5V 左右。
        
        3.1.2 布尔代数：
        
            所有的变量和函数的值都只能为 0 和 1 ，我们把它称为布尔代数-开关代数。
            
    3.2 基本数字逻辑电路：
    
        现在已经很少真正用一个一个的门电路来实现电路了，目前构造逻辑电路的基本组件是包含许多门的模块。
        
        3.2.1 集成电路：
            
            现在，已经不再有人制造和销售单个的门电路了，而是以集成电路或集成芯片为单位制造。
            一片集成电路是一小块方形的硅片，其尺寸和它用了多少个门来实现电路中的组件相关。
            
            对于芯片常见的有两类：引脚阵列封装(PGA) 和 触点阵列封装（LGA）。
            
            芯片都有输入信号和输出信号的微小的门延迟，包括信号在芯片中的传播时间和转换时间，
            一般为 100 皮秒到几纳秒。
            
            由于芯片都需要有管脚，为了控制芯片的大小，唯一的办法就是利用技术上的优势，设计出
            具有较高的门/管脚比的逻辑电路来。
            
        3.2.2 组合逻辑电路：
        
            许多数字逻辑的应用需要的电路，都有多个输入信号和多个输出信号，且输出信号是由
            输入信号的当前唯一确定，我们把它们叫作组合逻辑电路。
            
    3.3 内存：
    
        内存是用来存放计算机执行的指令和执行指令用到的数据。
        
        3.3.1 锁存器：
            
            要得到 1 位的内存，我们需要一个能以某种方式“记住” 上一个输入的值的电路。
            例如：SR锁存器电路。它有 S 和 R 两个输入信号，S 用来设置锁存器，R 用来重新设置锁存器。
            它的输出信号也是两个，一个是 Q, 另一个是 -Q。
            当 R=S=0, 根据其上一个状态 Q 的不同，锁存器有两个稳定的状态。
            将 S 置为 1 时，锁存器输出状态将为 1，而不管它的上一个状态是什么；
            将 R 置为 1，锁存器将输出 0 ，该电路能够“记忆”上次是置 S 还是置 R.
            我们正好用它这个特性来制造计算机的内存。
        
        3.3.2 触发器：
        
            许多电路中，必须在特定的时刻对某个信号采样并加以存储。我们把这种存储触发方式的内存叫作触发器。
            它的状态转变不是发生在时钟信号为 1 时，而是发生在时钟信号从 0 变为 1（上升沿）或从 1 边为 0 
            （下降沿）的时候。
        ...
        3.3.3 寄存器：
            ...
            多个触发器可以组合在一起，构成寄存器，来保存超过 1 位长的数据。
            8 个触发器连接在一起构成一个可存储 8 位数据的寄存器。
            
        3.3.4 内存组成：
        
            大量的内存采用的是每个内存字进行编址，是广泛使用的内存组成方式。
            
            内存系统有 4 个长为 3 位的字，对内存的每次读写操作都针对其中一个
            完整的 3 位字进行。尽管全部只有 12 位的内存空间无法和 8 位触发器相比。
            但它需要管脚数更少，而且，更重要，这种方案可以方便扩展成大容量的内存系统。
            值得注意的是，内存字的数量总是 2 是整数倍。
            
        3.3.5 内存芯片：
         
            大容量的内存芯片的内部结构采用的是 nxn 矩阵，通过行地址线和列地址线进行寻址。
            这种组成方式在减少了芯片的关键数的同时也减慢了芯片的寻址速度，因为它需要两个
            需知周期，一次是行地址寻址，一次是列地址寻址。
            
        3.3.6 RAM 和 ROM:
        
            RAM:
            
                内存即可读有可写的是 RAM(Random Access Memory)
            
                RAM 可分为静态RAM 和 动态 RAM 两大类。
                静态RAM(SRAM)内存用的是类似我们介绍的 D 触发器电路，只要不断地，存放在里面的数据
                可以永久保存，速度很快，静态RAM广泛应用在第2级的高速缓存中。
            
                动态的RAM(DRAM)使用的不是触发器，而是有晶体管和小电容组成的存储单元构成的阵列来存放数据，
                通过电容的充电和放电来存放0和1.存储密度高，主存储器几乎都是由DRAM构成的。
         
            ROM:
                
                ROM(Read-Only Mermory):存放的内存只能读，不能被改变或擦写，不管是有意还是无意。
    ...
    3.4 CPU 芯片和总线：
        ...
        3.4.1 CPU 芯片：
            ...
            目前所有的 CPU 都集成在单个芯片中，通过它的管脚与外部世界进行通信。
            CPU 芯片上的管脚可以分成三类：地址信号、数据信号、控制信号。这些管脚通过一组叫作
            总线的平行导线和内存、输入/输出接口的相应管脚相连。
